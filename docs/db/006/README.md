...
- **복합 인덱스**: 컬럼의 순서가 중요합니다. (선택도가 높은 컬럼을 먼저)
- **정렬 순서**: 오름차순, 내림차순도 쿼리 성능에 영향을 줍니다.
- **과도한 인덱스**: 쓰기 성능 저하와 디스크 공간 문제가 발생할 수 있습니다.
- **인덱스 갱신**: 데이터 변경 시 인덱스도 갱신되어 성능에 부하가 있을 수 있습니다.
- **비트맵 인덱스**: 낮은 카디널리티 컬럼의 경우 비트맵 인덱스 등의 특별한 인덱스 유형을 고려할 수 있습니다. (주로 읽기 작업이 많은 OLAP 환경에서 유용)
...
- **배치 작업**: 변경이 주로 되며 배치 작업이 많은 테이블의 경우 인덱스를 피합니다. (배치 작업 전후로 인덱스를 재구성하는 전략을 고려할 수 있음)
